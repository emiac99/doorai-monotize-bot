...
async def myreferrals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        lines.append(f"{referee} — {'Qualified' if qualified else 'Pending'} — joined {created[:19]}")
...
async def referral_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, r in enumerate(rows):
        tg, q = r
        lines.append(f"{i+1}. {tg} — {q} qualified")
...
async def get_referral_details_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows[:200]:
        ...
        lines.append(f"{referrer} -> {referee} — {'Q' if qualified else 'P'} — {created[:16]}")
...
async def top_clickers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return
    lines = [f"{i+1}. {r[0]} — {r[1]} clicks (last 7 days)" for i,r in enumerate(rows)]
...
def daily_report_worker(application):
    ...
    while True:
        ...
        try:
            rows = get_qualified_today()
            if not rows:
                text = f"🎯 Daily qualified users ({today_str()}): None"
            else:
                lines = []
                for i, r in enumerate(rows):
                    tg = r[0]; clicks = r[1]
                    ...
                    lines.append(f"{i+1}. @{uname} ({tg}) — {clicks} clicks")
                text = "🎉 Daily Qualified Users:\n" + "\n".join(lines)
